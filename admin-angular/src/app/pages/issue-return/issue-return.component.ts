  ...
  template: `
    <div class="space-y-6 pb-12 relative">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-1">Process checkouts, returns, and report lost books using Member ID or ISBN.</p>
      </div>
        ...
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4">
          ...
          <div class="space-y-3 text-xs">
            <div>
              ...
              <input [(ngModel)]="issueUserId" placeholder="e.g. STU-2026-042 or student&#64;slms.com" class="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white font-mono" />
            </div>
            <div>
            ...
            <tbody class="divide-y divide-slate-800">
              ...
              <tr *ngFor="let t of transactions" class="hover:bg-slate-800/30 transition-colors">
                ...
                <td class="p-4">
                  <span
                    ...
                    [ngClass]="{
                      ...
                      'bg-rose-500/20 text-rose-400': t.status === 'overdue' || t.status === 'lost',
                      'bg-slate-800 text-slate-400': t.status === 'returned'
                    }"
                  >
                    {{ t.status }}
                  </span>
                ...
                <td class="p-4 text-right space-x-2">
                  ...
                  <button *ngIf="t.status === 'issued' || t.status === 'overdue'" (click)="reportLost(t._id)" class="text-rose-400 font-semibold hover:text-rose-300">
                    Report Lost
                  </button>
                </td>
              </tr>
...
export class IssueReturnComponent implements OnInit {
  ...
  reportLost(transId: string) {
    if (!confirm('Mark this book as lost and charge lost penalty fine?')) return;
    this.api.markBookLost(transId, 500).subscribe({
      next: (res: any) => {
        this.returnMsg = `Book marked as Lost! Penalty Fine charged: ₹${res.data.fine?.amount}`;
        this.loadTransactions();
        setTimeout(() => (this.returnMsg = ''), 5000);
      },
      error: (err: any) => {
        this.returnErr = err.error?.message || 'Failed to report lost book.';
        setTimeout(() => (this.returnErr = ''), 5000);
      }
    });
  }
